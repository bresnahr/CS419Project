...
class TableList(npyscreen.MultiLineAction):
    ...
    def actionHighlighted(self, act_on_this, keypress):
		...
		self.parent.parentApp.selTableF = self.parent.parentApp.addForm('Tables', TableListDisplay, name='Select Table')
	
		del self.parent.parentApp.selDBType
                self.parent.parentApp.selDBType = self.parent.parentApp.addForm('MAIN', PostOrMySQL, name='Select DB')
...
'''**************************************************
   Class APIdisplay inherits MultiLineAction class

   Purpose:  display list of tables as list and define an action
   when one of the tables is selected
**************************************************'''
class APIlist(npyscreen.MultiLineAction):
    def __init__(self, *args, **keywords):
        super(APIlist, self).__init__(*args, **keywords)
    ...
    def actionHighlighted(self, act_on_this, keypress):
		...
		del self.parent.parentApp.myGridSet
                ...
                # save the name of selected table in settings object
                self.parent.parentApp.myGridSet.db_type = selectedDB
                # initialize TableMenuForm object attributes and switch to TableMenuForm
                self.parent.parentApp.getForm('Tables').api_name = selectedDB
                self.parent.parentApp.switchForm('Tables')


'''**************************************************
   Class PostOrMySQL inherits FiormMutt class

   Purpose:  Container for displaying choice of
   MySQL or PostgreSQL
**************************************************'''
class PostOrMySQL(npyscreen.FormMutt):

    # type of widget to be displayed
    MAIN_WIDGET_CLASS = APIlist
    # position of main_widget and status widget
    MAIN_WIDGET_CLASS_START_LINE = 2
    STATUS_WIDGET_X_OFFSET = 5
    ...
    def update_list(self):
	self.dbTypes = ['MySQL', 'PostGreSQL']
	self.wStatus1.value = ' Select Database API '
	self.wMain.values = self.dbTypes
	self.wMain.relx = 3
	self.wMain.display()
...
class TableOptionList(npyscreen.MultiLineAction):
    ...
    def actionHighlighted(self, act_on_this, keypress):
		...
		elif selection == 'Change Database API':
	...
	def create(self):
		...
		self.action = self.add(TableOptionList, max_height=4,
										...
										values = ['Add Row', 'Pagination Settings', 'Exit Application', 'Select Another Table',
										 'Change Database API'], scroll_exit = True
...
class MyApplication(npyscreen.NPSAppManaged):
	...
	def onStart(self):
		...
		self.selDBType = self.addForm('MAIN', PostOrMySQL, name='Select DB API')
		self.selTableF = self.addForm('Tables', TableListDisplay, name='Select Table')
	...
	***************************************************************************'''	
	def onInMainLoop(self):
		# when app leaves Table Menu Form (obj tabMenuF)
